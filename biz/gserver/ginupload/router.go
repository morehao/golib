...
func Register(group *gin.RouterGroup, fs *filestore.FileStore) {
	...
	f := group.Group("/files")
	{
		...
		f.GET("/:id/redirect", handleRedirectGetFileURL(fs))            // 302 重定向到文件 URL
		f.GET("/:id/serve", handleServeFileByID(fs))                    // 直接输出文件内容
		// 兼容查询参数形式（无文件 ID，仅 storage_uri 场景）
		f.GET("/redirect", handleRedirectGetFileURL(fs))
		f.GET("/serve", handleServeFileByID(fs))
	}
